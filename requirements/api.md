...
OpenWeather는 HTTP Status와 응답의 `cod` 값을 함께 반환한다. `cod`는 API와 오류 종류에 따라 문자열 또는 숫자일 수 있으므로 숫자로 정규화해 처리한다.
...
| Status                     | 원인                                                    | 클라이언트 처리                      |
| -------------------------- | ------------------------------------------------------- | ------------------------------------ |
| `400`                      | 좌표·기간·Parameter 형식 오류                           | 요청값 확인 안내                     |
| `401`                      | Key 누락, 잘못된 Key, Key 활성화 대기, 플랜 외 API 호출 | 설정 확인 안내                       |
| `404`                      | 장소 또는 리소스를 찾지 못함                            | 검색 결과 없음 처리                  |
| `429`                      | 분당·월간 호출 한도 초과                                | 재시도 중단, Cache 사용, 사용자 안내 |
| `500`, `502`, `503`, `504` | OpenWeather 서버 오류                                   | 지수 Backoff 후 제한 재시도          |
...
1. 사용자가 한글 도시명을 입력한다.
2. 이미 내려받은 도시 목록은 브라우저에서 즉시 필터링한다.
3. 원격 장소 검색은 300~500ms Debounce, 최소 2글자 조건으로 Geocoding API를 호출한다.
4. 새 입력이 발생하면 이전 요청은 `AbortController`로 취소한다.
5. 사용자가 장소 후보를 선택하면 `lat`, `lon`을 저장한다.
6. Current Weather API를 호출해 카드와 상세 화면을 갱신한다.
7. 예보 또는 대기질이 필요한 화면에서만 해당 API를 추가 호출한다.

### 9.2 Cache 정책

- 같은 좌표의 현재 날씨를 키로 Cache한다.
- Free Plan의 데이터 갱신 주기를 고려해 불필요한 반복 요청을 피한다.
- 개발 단계에서도 최소 10분 이내의 동일 위치 재요청은 Cache 결과를 우선 사용한다.
- 검색어 매 글자마다 Current Weather API를 호출하지 않는다.
- `429` 발생 시 즉시 반복 호출하지 않는다.
